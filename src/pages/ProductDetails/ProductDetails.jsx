              ...
              <div className="flex gap-3 m-4">
                <div className="hidden lg:block w-2/12">
                  <ul className="">
                    {productGallary.map((thumb, idx) => (
                      ...
                      >
                        <img
                          src={thumb.galleryImg}
                          className="w-[80px] h-[80px] border border-gray-400 rounded"
                        />
                      </li>
                    ))}
                  </ul>
                ...
                <div className="w-full lg:w-10/12">
                  <div className="mx-auto  overflow-hidden bg-white relative z-[999999]">
                    <div
                      className="whitespace-nowrap transition-all w-full  ease duration-[800ms] z-10 "
                      style={{
                    ...
                    >
                      {productGallary.map((sl) => {
                        return (
                          <div
                            className=" inline-block rounded-md  z-10 border border-gray-300 p-6"
                            key={sl.id}
                          >
                            <Magnifier
                              src={sl.galleryImg}
                              className="w-full lg:w-[450px] lg:h-[400px]"
                              mgShape="square"
                              mgWidth={180}
                              mgHeight={180}
                            />
                        ...
                        {productGallary.map((col, indx) => (
                          <li
                            ...
                            className={`transition-all duration-300 ease-in-out  cursor-pointer ${
                              indx === color
                                ? "p-[3px] rounded-full border border-gray-600"
                                : "p-[3px] border border-transparent"
                            }`}
                          >
                            <div
                        ...
                        {productGallary.map((sizeSlide, indx) => (
                          <>
                            <li
                              ...
                              className={`transition-all duration-300 ease-in-out cursor-pointer ${
                                indx === size ? "opacity-100" : "opacity-50"
                              }`}
                            >
                              <button
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center  flex-wrap">
              {products.slice(0, 8).map((product) => (
                <div
                  className="w-full sm:w-6/12 md:w-4/12 lg:w-3/12 xxl:w-1/5"
                  key={product._id}
                >
